...
There is also an ongoing effort for creating an encrypted root file system
while creating the image on CI, and later performing online re-encryption
during first boot, and you can track the progress at
https://github.com/foundriesio/meta-lmp/pull/868.
...
storing the Key Encryption Key (KEK) used for later decrypt the disk during
...
In order to avoid conflicts with the PKCS#11 token slot that is normally
used by ``aktualizr-lite``, a dedicated slot is required (currently hardcoded
to slot 1, with ``lmp`` as label).
...
Now install ``swtpm`` (if not already installed), and start the ``swtpm``
...
Run Qemu with the require extra TPM 2.0 related commands:
